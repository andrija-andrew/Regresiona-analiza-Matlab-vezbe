...
mean_x_i = NaN(k, 1);
for i = 1:k
    mean_x_i(i) = sum(sum(data(((i - 1) * s + 1):(i * s), :))) / (L * s);
end

mean_x_j = mean(data);
mean_x = sum(sum(data)) / N;
alpha_hat_i = mean_x_i - mean_x;
beta_hat_j = mean_x_j - mean_x;

mean_x_ij = NaN(k, L);
for i = 1:k
    mean_x_ij(i, :) = mean(data(((i - 1) * s + 1):(i * s), :));
end
...
Sw = 0;
for i = 1:k
    Sw = Sw + sum(sum((data(((i - 1) * s + 1):(i * s), :) - repmat(mean_x_ij(i, :), s, 1)) .^ 2));
end
...
F_reg_alpha = (Sr / (k - 1)) / (Sw / (N - k * L));
q_alpha = finv(cl, k - 1, N - k * L);
...
F_reg_beta = (Sc / (L - 1)) / (Sw / (N - k * L));
q_beta = finv(cl, L - 1, N - k * L);
